...
// qui sotto provo a scrivere il codice di slide 34 --> inizializzo un'istanza della struct Player, che specifica i valori di health e level
...
// qui scrivo un codice che istanzia una struct Test con dei valori di default che definisco con un metodo costruttore e provo a stampare se soddisfa o meno una certa condizione
...
// slide 48 (per i concetti sulle slide precedenti, ovvero 40-47 vedi tablet "concetti importanti rust")
// Qui sotto riporto alcuni esempi di utilizzo di una enum. Gli esempi sono stati proposti da Gemini

// A)  GESTIONE DI UN SISTEMA AD EVENTI
// In un sistema a eventi (come una UI o un server), non sai mai quale tasto premerà l'utente. Crei una funzione che accetta un solo parametro di tipo Game. Se usassi delle struct separate, dovresti avere una funzione diversa per ogni comando. Con l'enum, scrivi un unico "smistatore" (il match)

/* fn esegui_azione(azione: Game) {
    match azione {
        Game::Quit => process::exit(0),
        Game::Print(s) => println!("Log: {}", s),
        Game::Position { x, y } => muovi_player(x, y),
        Game::ChangeBackground(r, g, b) => set_color(r, g, b),
    }
} */

// B) MODELLARE STATI DI UN SISTEMA
// Pensa a una connessione di rete. Una connessione non può essere contemporaneamente "In attesa" e "Connessa".  L'enum garantisce che il tuo sistema sia sempre in uno e un solo stato valido. È impossibile avere una connessione che ha un IpAddr ma è contemporaneamente segnata come Disconnesso. 

/* enum Connessione {
    Disconnesso,
    InAttesa { tentativi: u32 },
    Connesso(IpAddr),
    Errore(String),
} */

// C)  GESTIONE DEGLI ERRORI
// In C, una funzione che può fallire restituisce -1. Ma cosa succede se il valore -1 è anche un risultato valido del calcolo? È un disastro. In Rust, usi Result: o hai il valore Ok(Dato), o hai l'errore Err(Messaggio). Non puoi mai confonderli
...
// slide 49  enumerazioni e clausole match --> ascolta anche registrazione telefono registrata in data 03-04-2026
// il punto chiave è che vengono legate delle variabili temporanee alla struttura (e.g Circle, Rectangle, Square ) per definire una serie di comportamenti alternativi --> DESTRUCTURING ASSIGNMENT


// slide 52 DESTRUTTURAZIONE
// per gestire un enum è possibile anche preoccuparsi solo di una o alcune varianti e ignorare tutte le altre; in questo modo non sono costretto a specificare il comportamento di tutti i casi. 

// if let <pattern> = <value>
// while let <pattern> = <value>

//ESEMPIO 1 slide 52, 53
/* enum Shape {
    Square {side : i32},
    Circle {radius: i32},
}
...
fn process(shape : Shape) {
    ...
    } // se la shape che gli ho dato in input è un quadrato allora stampi il valore del lato, altrimenti ingora 
}


fn main() {
    let s1 = Shape::Square { side : 5};
    let c1 = Shape::Circle { radius : 3};
    process(s1); // process stampa solo se s1 è un quadrato
    process(c1);
} */

//ESEMPIO 2 slide 54 
// il codice definisce un quadrato da un enum e raddoppia il valore del quadrato
// la scritta di un attributo con il punto esclamativo significa che l'attributo è valido su tutto il codice
/* #![allow(dead_code)]
#[derive(Debug)]
...
fn main() {
    let mut s1 = Shape::Square{side:5};
    println!("s1 before processing : {:?}", s1);
    process(&mut s1);
    println!("s1 after processing : {:?}", s1);
} */

// ---------------------------------------------------------------------------------------------------------------------------------------------------------------------

//slide 58 GENERIC ENUMS --> Option<T> --> Some(), None
// il tipo generico Option<T> mi permette di gestire i casi in cui un valore può non essere restituito correttamente da una funzione. Il risultato deve essere resituito nella scatola Some. T è un segnaposto che rappresenta un parametro di tipo. Quando viene restituito un option<T>, option è agnostico rispetto al tipo contenuto in T, anche se il programmatore deve specificare il tipo di T. Quando poi utilizzo some nella clausola match sto di fatto estraendo il dato dalla scatola. Some è la scatola. 

// questo esempio cerca di calcolare la divisione tra due valori, gestendo il caso denominatore = 0
...
fn main() {
    let result1 = divide(800, 34);
    let result2 = divide( 800, 0);

    match result1 {
        Some(value) => println!("Risultato trovato : {}", value),
        None => println!("Risultato non calcolabile."),
    }
...
// È possibile mettere in input un tipo generico in un'altra funzione anche nel caso in cui il tipo generico è un None. Questo permette di processare i dati in sicurezza senza preoccuparci che il dato potrebbe non esistere, in quanto è gestito con clausola match
// il programma prova a dividere due numeri e in caso di successo il risultato viene moltiplicato per 2, nonostante questo possa non esistere 

/* fn divide(numerator:f64, denominator:f64) -> Option<f64>{ // IMPORTANTE --> poichè il valore di ritorno è un tipo option, i valori che resituisci devono essere inclusi in varianti. 
    if denominator == 0.0 {
        None
    } else {
        Some(numerator / denominator) // Il valore che vuoi restituire deve essere specificato nella variante dell'enum generico Option. 
    }
}

fn multiply(x:Option<f64>) -> Option<f64>{
    match x {
        None => None,
...
fn main() {
    let division1 = divide(45.4, 3.6);
    let division2 = divide (34.3, 0.0);
    let result1 = multiply(division1);
    let result2 = multiply(division2);
    match result1 {
        None => println!("Impossibile calcolare il risultato."),
        Some(value) => println!("Il risultato finale è : {}", value),
    }
    match result2 {
        None => println!("Impossibile calcolare il risultato."),
        Some(value) => println!("Il risultato finale è : {}", value),
